...
function PopoverAplicarOutros({ talhoes, pares, idxOrigem, onAplicar, onClose }) {
  ...
  const disponiveis = talhoes.filter((_, i) => i !== idxOrigem);
  ...
  return (
    <div className="absolute z-50 right-0 top-8 bg-background border border-border rounded-xl shadow-xl p-3 w-64">
      ...
      <div className="space-y-1 max-h-48 overflow-y-auto mb-3">
        {disponiveis.map((t, i) => {
          const idxReal = talhoes.findIndex(x => x.id === t.id);
          const jaTemEste = pares[idxReal]?.arquivo?.name === arquivoOrigem?.name;
          return (
