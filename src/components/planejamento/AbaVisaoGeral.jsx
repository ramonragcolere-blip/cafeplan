...
import { calcularCustosFoliaresPorTalhao } from '@/lib/unidadesAplicacoesFoliares';
...
export default function AbaVisaoGeral({ talhoes, produtor, safra, planejamentosAdubacao, cronogramasFoliares = [] }) {
  ...
  const rows = useMemo(() => {
    const custosFoliares = calcularCustosFoliaresPorTalhao(cronogramasFoliares, talhoes, {
    ...
    const custoFoliarRha = custosFoliares[t.id]?.custoHa || 0;
    ...
    const custoTotal = custoAdubRha + custoFoliarRha + custoOpsRha + custoPosha;
    ...
    const custoSaca = prodEstimada ? custoTotal / prodEstimada : null;
      ...
      return { t, custoAdubRha, custoFoliarRha, custoOpsRha, custoPosha, custoTotal, prodEstimada, custoSaca };
    });
  }, [talhoes, produtor, safra, planejamentosAdubacao, cronogramasFoliares, operacoes, posColheitas]);
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Custo Foliar (R$/ha)</TableHead>
            ...
            <TableBody>
              {rows.length === 0 ? (
                <TableRow><TableCell colSpan={9} className="text-center py-10 text-muted-foreground">Nenhum talhão cadastrado</TableCell></TableRow>
              ) : rows.map(({ t, custoAdubRha, custoFoliarRha, custoOpsRha, custoPosha, custoTotal, prodEstimada, custoSaca }) => (
                <TableRow key={t.id}>
                  ...
                  <TableCell className="text-right">{fmtR(custoFoliarRha, 0)}</TableCell>
